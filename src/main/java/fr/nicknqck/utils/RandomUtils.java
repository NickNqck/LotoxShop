...
public class RandomUtils {
    public static int getRandomInt(int min, int max) {
        int toReturn =min + (LotoxShop.RANDOM).nextInt(max) - min;
        if (toReturn <min) {
            toReturn = min;
        }
        if (toReturn > max) {
    ...
    public static int getRandomDeviationValue(int value, int min, int max) {
        int i = max - min;
        int a = (LotoxShop.RANDOM).nextInt(i * 2) - i;
    ...
    public static String generateRandomString(int length, boolean color) {
        ...
        for (int i = 0; i < length; i++) {
            int randomIndex = LotoxShop.RANDOM.nextInt(ALLOWED_CHARACTERS.length());
            char randomChar = ALLOWED_CHARACTERS.charAt(randomIndex);

            if (color) {
                ChatColor randomColor;
                do {
                    randomColor = COLORS[LotoxShop.RANDOM.nextInt(COLORS.length)];
    ...
    public static boolean performActionWithProbability(double probability) {
        ...
        double randomValue = LotoxShop.RANDOM.nextDouble();
    ...
    public static boolean getOwnRandomProbability(double pourcentage) {
        ...
        double random = LotoxShop.RANDOM.nextDouble();
